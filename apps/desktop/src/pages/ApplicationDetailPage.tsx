...
import {
  ArrowLeft, BriefcaseBusiness, CalendarClock, Check, Circle, Clock3, ExternalLink,
  FileText, History, MapPin, Pencil, Plus, X,
} from "lucide-react";
import { useNavigate, useParams } from "react-router-dom";
import { Badge, Card } from "../components/ui";
import { useInterviewFlow } from "../hooks/useInterviewFlow";
import {
  createApplicationEvent, createApplicationTask, getApplicationDetail, hasLocalDatabase,
  setApplicationTaskStatus, updateApplicationDetail,
...
const stageTone = (stage: string): StatusTone => stage.includes("拒绝") ? "red" : stage.toLowerCase().includes("offer") ? "green" : stage.includes("面") ? "purple" : stage.includes("测评") ? "orange" : stage.includes("等待") ? "gray" : "blue";
...
export default function ApplicationDetailPage() {
  ...
  const { applications, refreshApplications } = useInterviewFlow();
  ...
  const addTask = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!detail) return;
    const data = new FormData(event.currentTarget);
    const input: CreateTaskInput = { title: String(data.get("title") || ""), description: optional(data.get("description")), priority: Number(data.get("priority")), dueAt: toUtc(optional(data.get("dueAt"))), remindAt: toUtc(optional(data.get("remindAt"))), applicationStage: optional(data.get("applicationStage")) };
    setSaving(true);
    try {
      if (hasLocalDatabase) {
  ...
  return <div className="page page-enter application-detail-page">
    <div className="application-detail-header">
      ...
      <button className="button button--primary detail-edit" onClick={() => setEditing(true)}><Pencil size={15} />编辑资料</button>
    </div>
    ...
    <div className="application-detail-grid">
      <div className="application-detail-main">
        ...
        <Card className="detail-section-card">
          ...
          <div className="event-timeline">{detail.events.length ? detail.events.map((item) => <div className="event-item" key={item.id}><span className={`event-dot event-dot--${item.sourceType}`} /><div><div className="event-title"><strong>{item.title}</strong><Badge tone="gray">{sourceText[item.sourceType] || item.sourceType}</Badge><time>{dateText(item.happenedAt)}</time></div>{item.content && <p>{item.content}</p>}{item.stageBefore && item.stageAfter && <div className="stage-change"><span>{item.stageBefore}</span><b>→</b><span>{item.stageAfter}</span></div>}</div></div>) : <div className="detail-empty">暂无事件记录</div>}</div>
        </Card>
      </div>

      <aside className="application-detail-side">
        <Card className="detail-section-card task-panel">
          <div className="section-heading"><div><CalendarClock size={18} /><span><h2>任务</h2><small>{detail.tasks.filter((item) => item.status !== "done").length} 项待完成</small></span></div><button className="icon-button" onClick={() => setAddingTask(true)} aria-label="新增任务"><Plus size={16} /></button></div>
          <div className="detail-task-list">{detail.tasks.length ? detail.tasks.map((task) => <div className={`detail-task ${task.status === "done" ? "is-done" : ""}`} key={task.id}><button className="detail-task-check" onClick={() => toggleTask(task)}>{task.status === "done" ? <Check size={14} /> : <Circle size={15} />}</button><div><strong>{task.title}</strong>{task.description && <p>{task.description}</p>}<span><Badge tone={task.priority === 3 ? "red" : task.priority === 2 ? "orange" : "gray"}>{priorityText(task.priority)}</Badge>{task.applicationStage && <em>{task.applicationStage}</em>}</span></div><time><Clock3 size={12} />{dateText(task.dueAt)}</time></div>) : <div className="detail-empty">还没有任务<br />为这次投递设置明确的下一步。</div>}</div>
          <button className="button button--secondary task-add-wide" onClick={() => setAddingTask(true)}><Plus size={14} />新增任务</button>
        </Card>
    ...
    {addingTask && <Modal title="新增任务" description={`关联到 ${detail.companyName} · ${detail.positionTitle}`} onClose={() => setAddingTask(false)}><form onSubmit={addTask}><div className="form-grid"><label className="full"><span>任务标题 *</span><input name="title" required autoFocus placeholder="例如：完成技术一面准备" /></label><label className="full"><span>描述</span><textarea name="description" rows={3} /></label><label><span>截止时间</span><input name="dueAt" type="datetime-local" /></label><label><span>提醒时间</span><input name="remindAt" type="datetime-local" /></label><label><span>优先级</span><select name="priority" defaultValue="2"><option value="3">高</option><option value="2">中</option><option value="1">普通</option></select></label><label><span>关联阶段</span><select name="applicationStage" defaultValue={detail.currentStage}><option value="">不限定</option>{!stages.includes(detail.currentStage) && <option>{detail.currentStage}</option>}{stages.map((stage) => <option key={stage}>{stage}</option>)}</select></label></div><ModalActions saving={saving} onCancel={() => setAddingTask(false)} /></form></Modal>}

    {addingEvent && <Modal title="补记事件" description="手动补充沟通、面试、备注等流程记录" onClose={() => setAddingEvent(false)}><form onSubmit={addEvent}><div className="form-grid"><label className="full"><span>事件标题 *</span><input name="title" required autoFocus placeholder="例如：HR 电话沟通" /></label><label className="full"><span>详细内容</span><textarea name="content" rows={4} /></label><label className="full"><span>发生时间</span><input name="happenedAt" type="datetime-local" defaultValue={new Date().toLocaleString("sv-SE").slice(0, 16)} /></label></div><ModalActions saving={saving} onCancel={() => setAddingEvent(false)} /></form></Modal>}
  </div>;
}
