...
export default function StructuredResumeSettings({ onMessage, onError }: { onMessage: (message: string) => void; onError: (message: string) => void }) {
  ...
  const importFiles = async () => {
    ...
    const confirmAiSend = aiConfigured && allowAiResume && (!promptBeforeAiSend || window.confirm(`将把选中的 ${paths.length} 份简历纯文本发送给已配置的 AI 服务进行结构化解析。是否继续？\n\n选择“取消”仍会导入文件，但只使用本地规则解析。`));
    setBusy(true); onError("");
    try {
      const imported = await Promise.all(paths.map((path) => importResumeProfile(path, confirmAiSend)));
      const items = await listResumeProfiles(); setProfiles(items);
      const latest = imported.at(-1)?.profile; if (latest) { setSelectedId(latest.id); setDraft(toDraft(latest)); }
      ...
      if (warnings.length) onError([...new Set(warnings)].join("；"));
      onMessage(`已添加 ${imported.length} 份简历${aiCount ? `，其中 ${aiCount} 份已由 AI 结构化` : ""}，请检查解析结果`);
  ...
  const save = async () => {
    ...
    setBusy(true); onError("");
    try { const previousId = selectedId; const updated = await updateResumeProfile(selectedId, serializeDraft(draft)); const items = await listResumeProfiles(); setProfiles(items); setSelectedId(updated.id); setDraft(toDraft(updated)); onMessage(updated.id === previousId ? "简历已保存" : "该简历已被投递引用，修改内容已自动保存为新版本"); } catch (reason) { onError(String(reason)); } finally { setBusy(false); }
  };
  ...
  const remove = async () => { if (!selectedId || !selected) return; const linked = selected.linkedApplicationCount > 0; if (!window.confirm(linked ? `这份简历关联了 ${selected.linkedApplicationCount} 个投递，将归档并保留历史引用。确定继续吗？` : "确定永久删除这份未关联的简历吗？")) return; try { await deleteResumeProfile(selectedId); const items = await listResumeProfiles(); setProfiles(items); const next = items.find((item) => !item.archivedAt) ?? items[0]; setSelectedId(next?.id ?? ""); setDraft(next ? toDraft(next) : undefined); onMessage(linked ? "简历已归档，历史投递仍保留该版本" : "简历已删除"); } catch (reason) { onError(String(reason)); } };
  ...
  return <div className="resume-page">
    ...
    <div className="resume-settings"><Card className="resume-list-card"><CardHeader title="简历版本" subtitle={`${profiles.filter((item) => !item.archivedAt).length} 份使用中 · ${profiles.filter((item) => item.archivedAt).length} 份已归档`} /><div className="resume-list">{profiles.map((profile) => <button type="button" key={profile.id} className={`${profile.id === selectedId ? "active" : ""} ${profile.archivedAt ? "is-archived" : ""}`} onClick={() => select(profile)}><span className="resume-list-icon">{profile.isPrimary ? <Check size={15} /> : profile.archivedAt ? <Archive size={15} /> : <UserRound size={15} />}</span><span><strong>{profile.name}</strong><small>{profile.archivedAt ? "已归档" : profile.targetDirection || profile.fileFormat?.toUpperCase() || "手动创建"} · {profile.linkedApplicationCount} 个投递</small></span><span className="resume-list-arrow">›</span></button>)}{!profiles.length && <div className="resume-empty"><UserRound size={28} /><p>还没有简历<br />点击右上角添加第一份</p></div>}</div><div className="resume-list-tip">被投递引用的版本修改时会自动派生新版本，历史记录不会被覆盖。</div></Card>
      <Card className="resume-editor-card"><CardHeader title={draft ? "编辑当前简历" : "开始建立简历"} subtitle={draft ? "每一段经历都是独立条目，可以单独添加或删除" : "上传简历后，这里会出现结构化编辑器"} />{draft ? <><div className="resume-editor-toolbar"><div className="resume-name-field"><span>当前版本名称</span><input value={draft.name} disabled={Boolean(selected?.archivedAt)} onChange={(event) => set("name", event.target.value)} placeholder="例如：后端开发 · 2026 春招" /></div><div className="resume-toolbar-actions"><button type="button" className="button button--secondary" onClick={duplicate}><Copy size={14} />复制版本</button><button type="button" className="button button--secondary" disabled={Boolean(selected?.archivedAt)} onClick={makePrimary}>{selected?.isPrimary ? <Check size={14} /> : null}{selected?.isPrimary ? "默认简历" : "设为默认"}</button><button type="button" className="button button--secondary" onClick={toggleArchived}>{selected?.archivedAt ? <RotateCcw size={14} /> : <Archive size={14} />}{selected?.archivedAt ? "恢复" : "归档"}</button>{!selected?.linkedApplicationCount && <button type="button" className="button button--secondary danger-text" onClick={remove}><Trash2 size={14} />删除</button>}</div></div><div className="resume-editor-note"><Check size={14} /> {selected?.linkedApplicationCount ? `已关联 ${selected.linkedApplicationCount} 个投递；保存修改时将自动创建新版本。` : "已完成本地解析；请检查每个条目中的时间、职责和量化结果。"}</div><div className="structured-resume-form">
        ...
        <Section title="项目经历" description="项目名称、职责、技术和成果分开记录" action={<AddButton onClick={() => set("projects", [...draft.projects, emptyProject()])} />}>{draft.projects.map((item, index) => <ProjectRow key={index} item={item} onChange={(value) => set("projects", replaceAt(draft.projects, index, value))} onRemove={() => set("projects", removeAt(draft.projects, index))} />)}</Section>
        ...
        {selected?.parsedText && <Section title="导入原文" description="用于核对 AI 解析结果"><details><summary>查看本地提取的纯文本</summary><pre className="jd-content">{selected.parsedText}</pre></details></Section>}
...
function Section({ title, action, children }: { title: string; description: string; action?: React.ReactNode; children: React.ReactNode }) { return <section className="resume-form-section"><div className="resume-section-heading"><h3>{title}</h3>{action}</div>{children}</section>; }
