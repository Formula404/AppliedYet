...
export default function MockInterviewPage() {
  ...
  return <div className="mock-layout">
    <Card className="mock-config"><CardHeader title="创建一场岗位模拟" subtitle={`预计 ${Math.max(10, count * 3)} 分钟`}/>
      ...
      <button className="button button--primary large" disabled={generating || (!experienceSelected && !(useAi && resumeReady))} onClick={start}><Play size={17}/>{generating ? "正在生成简历问题…" : `生成并开始 ${count} 题模拟`}</button>
    </Card>
