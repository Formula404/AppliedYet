...
export default function MockInterviewPage() {
  ...
  if (session && question) return <Card className="mock-runner">
    <div className="runner-head"><div><Badge tone="purple">模拟进行中</Badge><h2>{sessionApplication?.company} · {sessionApplication?.role}</h2></div><div className="runner-head-actions"><span>{questionIndex + 1} / {session.questions.length}</span><button className="button button--secondary" onClick={async () => { try { await updateSessionProgress(session.id, questionIndex); setSessionId(undefined); } catch (reason) { setAiError(String(reason)); } }}><LogOut size={14}/>退出并保存</button></div></div>
    <div className="runner-progress"><i style={{ width: `${((questionIndex + 1) / session.questions.length) * 100}%` }}/></div>
    <div className="runner-question"><small>{question.source}</small><h3>{question.prompt}</h3><textarea rows={9} value={question.answer} onChange={(event) => { void updateSessionAnswer(session.id, question.id, event.target.value).catch((reason) => setAiError(String(reason))); }} placeholder="记录你的完整回答；内容会实时保存，可稍后继续。"/><p>退出后可从“最近会话”恢复到当前题。</p>{aiError && <p className="field-error">{aiError}</p>}</div>
    <div className="runner-actions"><button className="button button--secondary" disabled={questionIndex === 0} onClick={() => { const next = questionIndex - 1; setQuestionIndex(next); void updateSessionProgress(session.id, next).catch((reason) => setAiError(String(reason))); }}><ChevronLeft size={16}/>上一题</button>{questionIndex < session.questions.length - 1 ? <button className="button button--primary" onClick={() => { const next = questionIndex + 1; setQuestionIndex(next); void updateSessionProgress(session.id, next).catch((reason) => setAiError(String(reason))); }}>下一题<ChevronRight size={16}/></button> : <button className="button button--primary" onClick={async () => { try { await completeSession(session.id); navigate(`/reviews?session=${session.id}`); } catch (reason) { setAiError(String(reason)); } }}><Check size={16}/>完成并进入复盘</button>}</div>
  </Card>;
